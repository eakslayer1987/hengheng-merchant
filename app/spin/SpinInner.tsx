...
export default function SpinInner() {
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      ...
      const duration = 4500; const startTime = performance.now(); const startRot = rotRef.current
      const tick = (now: number) => {
        ...
        if (t < 1) { rafRef.current = requestAnimationFrame(tick) }
          ...
          setTimeout(() => { localStorage.setItem('hh_result', JSON.stringify(data)); router.push('/result') }, 1200)
        }
      }
      ...
      <div className="w-full max-w-sm flex flex-col items-center gap-6">
        ...
        <div className="relative">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 z-10">
            <div className="w-0 h-0" style={{ borderLeft:'10px solid transparent', borderRight:'10px solid transparent', borderTop:'22px solid #fd1803', filter:'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }} />
          </div>
        ...
        {error && <div className="w-full bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-red-500 text-sm font-medium text-center">⚠️ {error}</div>}
        {result && <p className="text-gray-400 text-sm animate-pulse">กำลังไปหน้าผลรางวัล…</p>}
      </div>
    </div>
  )
}
