...
export async function POST(req: NextRequest) {
  ...
  const existingSpin = await queryOne(
    `SELECT id FROM spins WHERE user_id=? AND merchant_id=? AND DATE(created_at)=CURDATE()`,
    [user.id, merchant_id]
  )
  if (existingSpin)
  ...
  await execute(
    'UPDATE merchant_quota SET quota_used=quota_used+1 WHERE merchant_id=?', [merchant_id]
  )
  ...
  if (reward.type !== 'none') {
    ...
    await execute(
      `INSERT INTO coupons (code, user_id, reward_id, merchant_id, spin_id, expires_at) VALUES (?,?,?,?,?,?)`,
  ...
  return NextResponse.json({
    success: true,
    data: { reward, coupon_code, raffle_ticket_id: ticketResult.insertId, ticket_code: ticketCode,
      message: reward.type === 'none' ? 'เสียดายครั้งนี้ไม่ได้รางวัล แต่ได้รับ Ticket ลุ้นโชคใหญ่!' : `ยินดีด้วย! คุณได้รับ ${reward.name}` },
  })
}
