...
export default async function MerchantDashboardPage() {
  ...
  const recentSpins = await queryOne<{ count:number }>(
    `SELECT COUNT(*) AS count FROM spins WHERE merchant_id=? AND DATE(created_at)=CURDATE()`,
    [merchant.id]
  )
